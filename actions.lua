	...
	static = {
		...
		},
		{
			...
			handler = function()
				...
				if
						_G.networkTestCache
						and _G.networkTestCache.timestamp
						and (os.time() - _G.networkTestCache.timestamp) < 30
				then
				...
				local networkTestTask = hs.task.new("/bin/sh", function(exitCode, stdOut, stdErr)
					...
					if exitCode ~= 0 then
						...
						local pingLine =
								stdOut:match("round%-trip min/avg/max/stddev = [%d%.]+/([%d%.]+)/[%d%.]+/[%d%.]+ ms")
						if pingLine then
	...
	dynamic = {
		{
			...
			pattern = function(query)
				return string.match(query, "^%d+$") and (string.len(query) == 10 or string.len(query) == 13)
			end,
...

		{
			...
			enabled = true,
			pattern = function(query)
				return string.match(query, "^[A-Za-z0-9+/]*={0,2}$")
						and string.len(query) >= 4
						and string.len(query) % 4 == 0
			end,
			handler = function(query, context)
...

		{
			...
			pattern = function(query)
				...
				local hex =
						string.match(query, "^#?([a-fA-F0-9][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9][a-fA-F0-9])$")
				return hex ~= nil
			end,
			handler = function(query, context)
