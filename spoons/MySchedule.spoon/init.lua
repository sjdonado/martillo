...
function obj:setMenu(nextEvent, events)
      ...
      table.insert(menu, {
        ...
        fn = function()
          if event.meetingURL then
            ...
            hs.alert.show('Joining meeting: ' .. event.title, _G.MARTILLO_ALERT_DURATION)
          else
            hs.alert.show('No meeting URL found for: ' .. event.title, _G.MARTILLO_ALERT_DURATION)
          end
        end,
      })
    end
  ...
  table.insert(menu, {
    ...
    fn = function()
      ...
      hs.alert.show("Manually grant 'Full Calendar access' and restart Hammerspoon", _G.MARTILLO_ALERT_DURATION)
    end,
  })
